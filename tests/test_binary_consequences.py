...
from dcurves.dca import (
    _calc_tp_rate, _calc_fp_rate, _calc_prevalence, _create_initial_df,
    _calc_initial_stats, _calc_more_stats, _rectify_model_risk_boundaries
)
...
def test_case1_binary_test_pos_rate():
    ...
    r_benchmark_test_pos_rates_df = pd.DataFrame({
        ...
        "all": r_benchmark_results[
            r_benchmark_results["variable"] == "all"
        ]["test_pos_rate"].reset_index(drop=True),
        "none": r_benchmark_results[
            r_benchmark_results["variable"] == "none"
        ]["test_pos_rate"].reset_index(drop=True),
    })
    ...
    prevalence_value = _calc_prevalence(
        risks_df=rectified_risks_df, outcome=outcome
    )
    ...
    p_test_pos_rate_df = pd.DataFrame({
        "threshold": thresholds,
        "famhistory": initial_stats_df[
            initial_stats_df["model"] == "famhistory"
        ]["test_pos_rate"].reset_index(drop=True),
        "all": initial_stats_df[
            initial_stats_df["model"] == "all"
        ]["test_pos_rate"].reset_index(drop=True),
        "none": initial_stats_df[
            initial_stats_df["model"] == "none"
        ]["test_pos_rate"].reset_index(drop=True),
    })

    for model in ["famhistory", "all", "none"]:
        assert r_benchmark_test_pos_rates_df[model].round(decimals=6).equals(
            p_test_pos_rate_df[model].round(decimals=6)
        )
...
def test_case1_binary_calc_initial_stats():
    ...
    for model in ["all", "none", "famhistory"]:
        for stat in ["test_pos_rate", "tp_rate", "fp_rate"]:
            model_df = initial_stats_df[
                initial_stats_df.model == model
            ][stat].round(decimals=6).reset_index(drop=True)
            benchmark_df = r_benchmark_results_df[
                r_benchmark_results_df.variable == model
            ][stat].round(decimals=6).reset_index(drop=True)
...
def test_case1_binary_calc_more_stats():
    ...
    for model in ["all", "none", "famhistory"]:
        assert final_dca_df[
            final_dca_df.model == model
        ]["net_benefit"].round(decimals=6).reset_index(drop=True).equals(
