...
def test_python_joint():
    ...
    df_cancer_dx["high_risk"] = [
        1 if risk_group == "high" else 0 for risk_group in df_cancer_dx["risk_group"]
    ]

    df_cancer_dx["joint"] = [
        1 if (risk_group == "high" or cancerpredmarker > 0.15) else 0
        for risk_group, cancerpredmarker in zip(
            df_cancer_dx["risk_group"], df_cancer_dx["cancerpredmarker"]
        )
    ]
    ...
    df_cancer_dx["conditional"] = [
        ...
        if (
            risk_group == "high"
            or (risk_group == "intermediate" and cancerpredmarker > 0.15)
        )
        else 0
        for risk_group, cancerpredmarker in zip(
            df_cancer_dx["risk_group"], df_cancer_dx["cancerpredmarker"]
...
def test_python_dca_case_control():
    ...
    # Summarize Data
    medians = (
        df_case_control.drop(columns="patientid").groupby(["casecontrol"]).median()
    )
    # print('\n', medians.to_string())
