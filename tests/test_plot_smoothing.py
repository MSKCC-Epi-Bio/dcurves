...
from dcurves.plot_graphs import plot_graphs, _plot_net_benefit, _plot_net_intervention_avoided
...
def test_plot_with_invalid_smoothed_data(sample_data_df, color_names):
    ...
    with pytest.raises(ValueError):
        _plot_net_benefit(sample_data_df, y_limits=(-0.05, 1), color_names=color_names, smoothed_data=invalid_smoothed_data)
...
def test_smoothed_data_with_unmatched_model_names(sample_data_df, color_names):
    unmatched_smoothed_data = {"Model3": np.array([[0, 0.1], [0.5, 0.5], [1, 0.9]])}  # Model3 does not exist in sample_data_df
    with patch('matplotlib.pyplot.plot') as mock_plot, \
         patch('matplotlib.pyplot.show'):
        _plot_net_benefit(sample_data_df, y_limits=(-0.05, 1), color_names=color_names, smoothed_data=unmatched_smoothed_data)
        assert mock_plot.call_count == len(sample_data_df['model'].unique())  # Only plots for existing models
...
def test_multiple_smoothed_lines_per_model(sample_data_df, color_names):
    ...
    single_line_smoothed_data = {
        "Model1": np.array([[0, 0.1], [0.5, 0.5], [1, 0.9]])
    }
    
    with patch('matplotlib.pyplot.plot') as mock_plot, \
         patch('matplotlib.pyplot.show'):
        _plot_net_benefit(sample_data_df, y_limits=(-0.05, 1), color_names=color_names, smoothed_data=single_line_smoothed_data)
...
def test_smoothed_data_with_missing_points(sample_data_df, color_names):
    ...
    with patch('matplotlib.pyplot.plot') as mock_plot, \
         patch('matplotlib.pyplot.show'):
        _plot_net_benefit(sample_data_df, y_limits=(-0.05, 1), color_names=color_names, smoothed_data=missing_points_smoothed_data)
        assert mock_plot.called
...
def test_empty_smoothed_data(sample_data_df, color_names):
    with patch('matplotlib.pyplot.plot') as mock_plot, \
         patch('matplotlib.pyplot.show'):
        _plot_net_benefit(sample_data_df, y_limits=(-0.05, 1), color_names=color_names, smoothed_data={})
        assert mock_plot.call_count == len(sample_data_df['model'].unique())  # Fallback to plotting raw data
