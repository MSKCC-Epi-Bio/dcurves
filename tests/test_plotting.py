...
def test_invalid_graph_type():
    ...
    with pytest.raises(
        ValueError,
        match="graph_type must be one of 2 strings: net_benefit, net_intervention_avoided",
    ):
...
def test_fewer_color_names_than_models():
    ...
    with pytest.raises(
        ValueError,
        match="More predictors than color_names, please enter more color names in color_names list and try again",
    ):
