...
def test_case2_surv_risk_rate_among_test_positive():
    ...
    for model in ["all", "none", "cancerpredmarker"]:
        risk_rate_among_test_pos_series = _calc_risk_rate_among_test_pos(
            risks_df=rectified_risks_df,
            outcome=outcome,
            model=model,
            thresholds=thresholds,
...
def test_tut_pr_failure18_tp_rate():
    ...
    df_time_to_cancer_dx.loc[:, "pr_failure18"] = [
        1 - val for val in cph_pred_vals.iloc[0, :]
    ]
    ...
    comp_df = pd.concat([initial_stats_df, r_stats_df], axis=1).drop(
        ["harm", "index", "label", "n", "net_benefit"], axis=1
    )
    ...
    round_dec_num = 7
    assert (
        comp_df["test_pos_rate"]
        .round(decimals=round_dec_num)
        .equals(comp_df["r_test_pos_rate"].round(decimals=round_dec_num))
    )
